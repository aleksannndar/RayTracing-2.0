...
DiscoRoom::DiscoRoom(int nBalls, int nLights, int seed) {
	...
	for (int i = 0; i < nBalls; i++) {
		bodies.push_back(std::make_shared<Uniform>(std::make_shared<Sphere>(Vec3::randomVec().Z0toMP(), 0.1), defaultMaterial.setDiffuse(Vec3(randomDouble(), randomDouble(), randomDouble())).setSpecular(Vec3(1.0,1.0,1.0)).setShininess(32)));
	}
	srand(2 * seed + 1);
